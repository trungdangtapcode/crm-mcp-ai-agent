...
enabling agentic capabilities for AI applications with SSE transport support.
"""
...
try:
    ...
    logger.info("FastMCP library available - SSE transport enabled")
except ImportError:
    FASTMCP_AVAILABLE = False
    import httpx
    logger = logging.getLogger("mcp_client")
    logger.warning("FastMCP library not available - falling back to HTTP transport")
...
class MCPClient:
    ...
    def __init__(self, base_url: str, timeout: int = 30):
        ...
        if FASTMCP_AVAILABLE:
            # For SSE transport, we need to use the correct endpoint
            # FastMCP SSE servers typically expose SSE at the root or /sse endpoint
            try:
                self.transport = SSETransport(url=base_url)
                self.fastmcp_client = Client(self.transport)
                logger.info(f"Initialized FastMCP client with SSE transport for {base_url}")
    ...
    async def initialize(self, retry_count=3, use_fallback=True):
        ...
        """
        # Try FastMCP client with SSE transport first
        if FASTMCP_AVAILABLE and self.fastmcp_client:
            for attempt in range(retry_count):
                try:
                    logger.info(f"Connecting to MCP server via SSE (attempt {attempt+1}/{retry_count})")
                    ...
                    async with self.fastmcp_client as client:
                        ...
                        logger.info(f"Successfully loaded {len(self.available_tools)} tools via SSE")
                        self.connection_status = "connected"
                        return self.available_tools
                        
                except Exception as e:
                    logger.error(f"SSE connection failed (attempt {attempt+1}/{retry_count}): {str(e)}")
                    if attempt < retry_count - 1:
                        await asyncio.sleep(2)
        
        # If SSE failed or not available, use fallback
        if use_fallback:
            logger.warning("Using fallback tools since SSE connection failed")
    ...
    async def call_tool(self, tool_name: str, **params) -> Any:
        ...
        # Try FastMCP client with SSE first
        if FASTMCP_AVAILABLE and self.fastmcp_client:
            try:
                async with self.fastmcp_client as client:
                    result = await client.call_tool(tool_name, **params)
                    logger.info(f"Tool {tool_name} executed successfully via SSE")
                    return result
            except Exception as e:
                logger.error(f"SSE tool call failed for {tool_name}: {str(e)}")
    ...
    def get_tools_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "transport_type": "SSE" if (FASTMCP_AVAILABLE and self.fastmcp_client) else "HTTP",
            "categories": {
                category: {
                    "count": len(tools),
